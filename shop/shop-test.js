console.log('shop-test loaded in', location.href);

// Guard：只在 popup 页面运行
if (!document.querySelector('.shop-window')) {
  console.log('shop-test: not running in popup, exiting');
} else {
  // 不要在文件顶层立刻发送注入请求，或者把发送放到用户交互后
  // chrome.runtime.sendMessage({ action: "injectShopScript" });
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (closeBtn) {
      closeBtn.addEventListener('click', () => {
        window.close(); // 或 window.closeShop()，视你的逻辑而定
      });
    }
